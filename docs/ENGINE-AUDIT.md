...
|----|-----|------|------|--------|
| `COMPAT-1` | **H** | Compatibility | **`UI/InGame/TopPanel.lua` collides with Corporations (BNW).** **Both mods are Enabled=1 Activated=1 right now**, so one top bar is already losing silently — VFS replacement is last-one-wins, no error shown. Theirs: 1343 lines, `include("Corp_UI.lua")`, corp revenue in GPT. Ours: 860 lines, g/s/c treasury, mod income, capital-market tooltip. **Preferred fix — coexist:** modinfo `<References>` is a supported, widely-used mechanism (39 installed mods use it) meaning *load after this mod if present*; referencing Corporations makes our TopPanel deterministically win, and our TopPanel then adds their revenue line when `GetCorporationRevenue` is available. Both mods keep their features. Alternatives: drop the replacement (loses g/s/c in the top bar entirely) or document the incompatibility. **Diagnostic:** g/s/c visible in the top bar ⇒ ours is winning; plain integer gold ⇒ theirs is. | open |
