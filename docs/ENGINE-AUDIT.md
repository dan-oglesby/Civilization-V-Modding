...
|----|-----|------|------|--------|
| `COMPAT-1` | **H** | Compatibility | `UI/InGame/TopPanel.lua` collided with Corporations (BNW); both were Enabled+Activated, so one top bar was losing silently. **FIXED 2026-07-06:** modinfo now declares a `<References>` entry for Corporations (a reference, not a dependency - the mod stays optional), so we deterministically load last and win the VFS; `TopPanel.lua` then pcall-includes `Corp_UI.lua` and folds `GetCorporationRevenue` into the science check, the gold-per-turn figure and the tooltip income breakdown. Both mods keep their features. | **done** |
